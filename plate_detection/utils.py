...
def reconstruct(I, Iresized, Yr, lp_threshold):
    ...
    print("Len final extracted lp: ", len(final_labels))
    if len(final_labels):
...
def detect_lp(model, I, max_dim, lp_threshold):
    ...
    Yr = np.squeeze(Yr)
    print(Yr.shape)
    L, TLp, lp_type, Coordinate = reconstruct(I, Iresized, Yr, lp_threshold)
    return L, TLp, lp_type, Coordinate
...
def draw_coordinate(image, coordinates):
    for coordinate in coordinates:
        image = cv2.line(image, (int(coordinate[0][0]), int(coordinate[1][0])), (int(
            coordinate[0][1]), int(coordinate[1][1])), (0, 0, 255), 2)
        image = cv2.line(image, (int(coordinate[0][0]), int(coordinate[1][0])), (int(
            coordinate[0][3]), int(coordinate[1][3])), (0, 0, 255), 2)
        image = cv2.line(image, (int(coordinate[0][1]), int(coordinate[1][1])), (int(
            coordinate[0][2]), int(coordinate[1][2])), (0, 0, 255), 2)
        image = cv2.line(image, (int(coordinate[0][2]), int(coordinate[1][2])), (int(
            coordinate[0][3]), int(coordinate[1][3])), (0, 0, 255), 2)
    return image
def draw_box(image, boxes):
    for box in boxes:
        cv2.rectangle(image, (box[0], int(box[1])), (int(box[2]), int(box[3])), (255, 0, 0), 5)
    return image

def image_array_to_base64(image):
    _, im_array = cv2.imencode('.jpg', image)
